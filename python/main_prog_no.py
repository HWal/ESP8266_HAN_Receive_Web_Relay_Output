...
def main():
    ...
    info_text1 = Text(Point(400, 35), "PLOTTE DATA FRA KAIFA MA304H3E SMART METER")
    info_text1.draw(win_in)
    folder_text = Text(Point(92, 85), "Sti til loggfiler:")
    folder_text.draw(win_in)
    date_text1 = Text(Point(137, 150), "Dato nr. 1 (YYYY-MM-DD):")
    date_text1.draw(win_in)
    power_text1 = Text(Point(583, 150), "Effektkurve (J/N):")
    power_text1.draw(win_in)
    volt_text1 = Text(Point(600, 180), "Spenningskurve (J/N):")
    volt_text1.draw(win_in)
    current_text1 = Text(Point(583, 210), "Stromkurve (J/N):")
    current_text1.draw(win_in)
    date_text2 = Text(Point(137, 260), "Dato nr. 2 (YYYY-MM-DD):")
    date_text2.draw(win_in)
    power_text2 = Text(Point(583, 260), "Effektkurve (J/N):")
    power_text2.draw(win_in)
    volt_text2 = Text(Point(600, 290), "Spenningskurve (J/N):")
    volt_text2.draw(win_in)
    current_text2 = Text(Point(583, 320), "Stromkurve (J/N):")
    current_text2.draw(win_in)
    info_text2 = Text(Point(132, 400), "Skriv inn verdier og klikk:")
    ...
    rect1.draw(win_in)
    finish_text = Text(Point(675, 400), "Avslutt")
    ...
    # set default values
    folder = "C:\\Users\\HelgeN550JK\\Desktop\\AMS\\python\\logs\\"
    date1 = "2019-01-20"
    date2 = "2019-01-30"
